...
class WC_Form_Handler {
	...
	public static function init() {
		...
		add_action( 'wp_loaded', array( __CLASS__, 'update_cart_action' ), 10 );
	...
	public static function update_cart_action() {	
		if ( ! empty( $_POST['apply_coupon'] ) && ! empty( $_POST['coupon_code'] ) ) {
			...
			if ( $cart_item = WC()->cart->get_cart_item( $cart_item_key )) {
				...
				{
					foreach($cart_item['variation'] as $vari){
						$variation_name = $vari;
					}
				}
				
				$item_removed_title = apply_filters( 'woocommerce_cart_item_removed_title', $product ? sprintf( _x( '&ldquo;%s&rdquo;', 'Item name in quotes', 'woocommerce' ), $product->get_name()." - ".$variation_name ) : __( 'Item', 'woocommerce' ), $cart_item );
	...
	private static function add_to_cart_handler_variable( $product_id ) {
		...
		if ( $passed_validation && WC()->cart->add_to_cart( $product_id, $quantity, $variation_id, $variations ) !== false ) {
			...
			wc_add_to_cart_message( array( $product_id => $quantity ), true, false, $variations );
			return true;
		}
