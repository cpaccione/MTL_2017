 ...
 */
function wc_add_to_cart_message( $products, $show_qty = false, $return = false, $myvariations = "") {
	$titles = array();
	$count  = 0;
	...
	$variation_name = "";
	if ( !is_null($myvariations) && is_array($myvariations) )
	{
		foreach ( $myvariations as $tmp ) {
			$variation_name =  $tmp;
		}
	}
	
	foreach ( $products as $product_id => $qty ) {
		$titles[] = ( $qty > 1 ? absint( $qty ) . ' &times; ' : '' ) . sprintf( _x( '&ldquo;%s&rdquo;', 'Item name in quotes', 'woocommerce' ), strip_tags( get_the_title( $product_id ) ) . " - ". $variation_name );
		$count += $qty;	
	}
